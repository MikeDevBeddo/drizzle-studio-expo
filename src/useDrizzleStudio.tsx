 ...
 */
export function useDrizzleStudio(db: SQLite.SQLiteDatabase | null): void {
  ...
  const dbRef = useRef<SQLite.SQLiteDatabase | null>(db);
  dbRef.current = db;

  const pendingQueries = useRef<QueryEvent[]>([]);
  const pendingTransactions = useRef<TransactionEvent[]>([]);
  ...
  useEffect(() => {
    ...
    const subscriptions: EventSubscription[] = [
      client.addMessageListener('query', (event: QueryEvent) => {
        emitDebug(client, {
          kind: 'query:recv',
          id: event.id,
          sql: event.sql,
      ...
      client.addMessageListener('transaction', (event: TransactionEvent) => {
        emitDebug(client, {
